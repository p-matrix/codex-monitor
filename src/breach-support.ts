...
/** Serializable breach state for cross-process persistence (Claude Code Command Hook). */
...
export class BreachSupport {
  ...
  // ── File-based Persistence (Claude Code Command Hook architecture) ──
  // Each hook invocation is a separate process, so state must be persisted to disk.
  // Follows the same atomic-write pattern as state-store.ts.

  /**
   * Save breach state to ~/.pmatrix/sessions/{sanitized_session_id}_breach.json.
   * Atomic write: tmp → rename. Fail-open: I/O failure silently ignored.
   */
  saveState(sessionId: string): void {
    try {
      const dir = path.join(os.homedir(), '.pmatrix', 'sessions');
      fs.mkdirSync(dir, { recursive: true });

      const filepath = BreachSupport._breachFilePath(sessionId);
      ...
      const state: BreachState = {
        ...
        fileModCount: this.fileModCount,
  ...
  /**
   * Load persisted breach state and create a BreachSupport instance.
   * Falls back to a fresh instance if file is missing or corrupt (graceful degradation).
   */
  static loadOrCreate(agentId: string, sessionId: string): BreachSupport {
    const instance = new BreachSupport(agentId);
    try {
      const filepath = BreachSupport._breachFilePath(sessionId);
      if (!fs.existsSync(filepath)) return instance;

      const raw = fs.readFileSync(filepath, 'utf-8');
      const state = JSON.parse(raw) as BreachState;

      instance.approvalRecords = state.approvalRecords ?? [];
      instance.blockedActions = state.blockedActions ?? [];
      instance.toolCallCount = state.toolCallCount ?? 0;
      instance.fileModCount = state.fileModCount ?? 0;
      instance.errorsCount = state.errorsCount ?? 0;
      instance.deniedCount = state.deniedCount ?? 0;
      instance.sessionStartTime = state.sessionStartTime ?? Date.now();
    } catch {
      // Graceful degradation: return fresh instance
    }
    return instance;
  }

  /**
   * Delete breach state file (called at session_end cleanup).
   */
  static deleteState(sessionId: string): void {
    try {
      const filepath = BreachSupport._breachFilePath(sessionId);
  ...
  private static _breachFilePath(sessionId: string): string {
    const safe = sessionId.replace(/[^a-zA-Z0-9\-_]/g, '_').slice(0, 128);
    return path.join(os.homedir(), '.pmatrix', 'sessions', `${safe}_breach.json`);
  }
